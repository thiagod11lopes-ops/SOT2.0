...
import { Building2, Ambulance, Upload } from "lucide-react";
...
export function SaidasLayout() {
  ...
  function handleFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        let rows =
          Array.isArray(parsed) ? normalizeDepartureRows(parsed) : mapSotBackupJsonToDepartures(parsed);
        if (rows.length === 0) {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[hsl(var(--background))]">
      <header
        className="sticky top-0 z-20 border-b border-[hsl(var(--border))]/90 bg-[hsl(var(--card))]/85 px-4 pb-3 pt-[calc(0.75rem+var(--safe-top))] backdrop-blur-xl"
      ...
      >
        <div className="mx-auto flex max-w-lg items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-lg font-bold tracking-tight text-[hsl(var(--foreground))]">Saídas</h1>
          </div>
          <button
            type="button"
            onClick={() => fileRef.current?.click()}
            className="flex h-11 min-w-[2.75rem] items-center justify-center gap-2 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--muted))]/40 px-3 text-sm font-medium text-[hsl(var(--foreground))] transition active:scale-[0.98] min-[400px]:px-4"
          >
