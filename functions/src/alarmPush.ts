/**
 * Alarmes push agendados — depende de Secret Manager (WEB_PUSH_VAPID_*).
 * Volte a exportar em `index.ts` (`export { processMobileAlarmPush } from "./alarmPush.js"`)
 * depois de: `firebase functions:secrets:set WEB_PUSH_*` e `firebase deploy --only functions`.
 */
...
/** Forma mínima compatível com `web-push` (import dinâmico em runtime). */
type MobilePushSub = { endpoint: string; keys: { p256dh: string; auth: string } };
...
async function loadWebPushConfig(): Promise<boolean> {
...
async function readPushSubscriptions(): Promise<Map<string, MobilePushSub[]>> {
  const db = getFirestore();
  const snap = await db.collection("sot_mobile_push_subscriptions").where("enabled", "==", true).get();
  const out = new Map<string, MobilePushSub[]>();
  snap.forEach((d) => {
...
async function sendPushToDriver(
  ...
  title: string,
  body: string,
  subscriptionsByDriver: Map<string, MobilePushSub[]>,
): Promise<void> {
